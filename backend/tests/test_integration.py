...
"""
    ...
    def test_search_functionality(self, test_config, sample_courses_dir):
        ...
        ), "Search for 'variables Python' returned no results"
        assert any(
            "variable" in doc.lower() for doc in results.documents
        ...
        results = system.vector_store.search(
            query="Python", course_name="Python Programming", lesson_number=1
        )
        assert not results.is_empty(), "Lesson-filtered search returned no results"
        assert all(
            meta.get("lesson_number") == 1 for meta in results.metadata
    ...
    def test_course_search_tool_execution(self, test_config, sample_courses_dir):
        ...
        assert (
            "[Python Programming Basics" in result
        ), "Result doesn't show course context"
        ...
        result = system.search_tool.execute(
            query="test", course_name="Non-Existent Course XYZ"
        )
    ...
    def test_query_with_mocked_ai(
        self, mock_anthropic, test_config, sample_courses_dir
    ):
...
""" + " ".join(
                [f"This is sentence {j} in lesson {i+1}." for j in range(100)]
            )
